{ config, ... }:
{
  ...
  home.file.".clojure/deps.edn".text = ''
    {:aliases {:user {:extra-deps {global/user {:local/root "${config.metadata.home-dir}/.clojure/injections"}
                                   djblue/portal {:mvn/version "${config.metadata.clojure-portal.version}"}}}
  ...
  home.file.".lein/profiles.clj".text = ''
    {:user {:dependencies [[djblue/portal "${config.metadata.clojure-portal.version}"]]
            :injections [(load-file "${config.metadata.home-dir}/.clojure/injections/src/tap.clj")
